...
@testset "Test periodicity" begin
    samples = (-1.01:.1:1) * 6  # avoid using pi3 and 0 to prevent degenerate cases.
    @testset "Testing periodicity for ω=$omega." for omega in exp.((0:2) .* 2im * pi / 3) .* pi3
        for (x, y) in Iterators.product(samples, samples)
            z = Complex(x, y)
            @test isapprox(Dixon.center(z + omega, DixonElliptic), Dixon.center(z, DixonElliptic); atol=1.e-10)
