...
# ╔═╡ 0c4425bc-ef0a-4abc-8191-f78c83ef2d8f
using Symbolics

# ╔═╡ 4a891a58-893e-411c-abf2-57922f840f6c
using TaylorSeries

# ╔═╡ 5835df5b-13ab-4c13-8ad4-f94d8260a19b
using GLMakie

# ╔═╡ e7c1f142-080c-4ba3-a704-dac312919ef3
using SpecialFunctions: beta

# ╔═╡ 83c4f621-5670-4c10-beea-e59dbce43302


# ╔═╡ 79122c95-e6b6-4884-8ae7-a249f7e1108b


# ╔═╡ 2a081a2d-1f3c-47d7-af88-20fadfece6e8
pi3 = beta(1//3, 1//3)

# ╔═╡ 014326c5-13cf-4b43-914e-c9aedbfac51a
table = Dict(
	...
	-1//4 => (cm=(1+sqrt(3) + sqrt(2*sqrt(3))) / 2, sm=(-1-sqrt(3 + 2 * sqrt(3))) / cbrt(4), broken=false),
	-1//12 => (cm=(-1+sqrt(3) + sqrt(2*sqrt(3))) / (2*cbrt(2)), sm=(-1+sqrt(3) - sqrt(2*sqrt(3))) / (2*cbrt(2)), broken=false),
	1//12 => (cm=(-1+sqrt(3 + 2*sqrt(3))) / cbrt(4), sm=(1+sqrt(3) - sqrt(2*sqrt(3))) / 2, broken=false),
	1//4 => (cm=(1+sqrt(3) - sqrt(2*sqrt(3))) / 2, sm=(-1+sqrt(3 + 2 * sqrt(3))) / cbrt(4), broken=false),
	5//12 => (cm=(-1+sqrt(3) - sqrt(2*sqrt(3))) / (2*cbrt(4)), sm=(-1+sqrt(3) + 2 * sqrt(2*sqrt(3))) / (2*cbrt(4)), broken=true),
	7//12 => (cm=(-1-sqrt(3 + 2 * sqrt(3))) / cbrt(4), sm=(1+sqrt(3) + sqrt(2*sqrt(3))) / 2, broken=false),
)

# ╔═╡ edb00d38-4467-4998-a20b-7f248f7bfef2
T = Real

# ╔═╡ 2727b327-2cfa-4ed2-81c9-965fc11f470a
@variables z::T

# ╔═╡ f31c04e9-a3b2-443a-b727-c97e667852e2
@syms cm(z::T)::T sm(z::T)::T
# :cm

# ╔═╡ 84239d23-56b7-410e-9290-ea92111d523b
# :sm

# ╔═╡ 9e717ec6-8ac9-4adb-9678-e325c304c257
f = 2 * cm(z) * sm(z)

# ╔═╡ bb208747-acbf-4612-b1dc-675ae9c67270
dz = Differential(z)

# ╔═╡ c75b3f41-910b-4080-9318-3403030923ec
dz(f) |> expand_derivatives

# ╔═╡ 3340038c-563b-4802-a42a-64a42b0ff5da
# Symbolics.erivative(::typeof(cm), args::NTuple{1,Any}, ::Val{1}) = -sm^2(args[1])

# ╔═╡ dd557cf1-9b53-4d7b-b00a-d0f795c6f080
# Symbolics.derivative(::typeof(sm), args::NTuple{1,Any}, ::Val{1}) = cm^2(args[1])

# ╔═╡ 9fdfb1d8-a0eb-4ac4-971d-34e8727ead65
# dz(sm(z)) |> expand_derivatives

# ╔═╡ 7eb0e449-d08c-47a5-ac03-c735b83dfd7c
subs = Dict(
	dz(cm(z))=>-sm(z)^2,
	dz(sm(z))=>cm(z)^2
);

# ╔═╡ a987bf93-8e78-4636-8f13-d50c34ae3ac0
begin
	n = 100
	fp = [Symbolics.substitute(f |> expand_derivatives, subs)]
	for k in 1:n
		push!(fp, Symbolics.substitute(dz(fp[end]) |> expand_derivatives, subs) * (1 / k))
	end
end

# ╔═╡ 30a422fa-4d49-452b-8714-4f0784f7f570
fp

# ╔═╡ fd33df3c-3b89-4206-9c21-d9299c4b4f9c


# ╔═╡ 24e8058a-32fe-4a6e-bb65-d9466ec5885d


# ╔═╡ 64794b7d-99f4-4b56-be94-d34d92c6cb8b
function get_taylor(fp, z_0::Number)
	evaluated = Symbolics.value.(map(sub -> substitute(
		sub, 
		Dict(cm(z) => table[z_0][:cm], sm(z) => table[z_0][:sm])
	), fp))
	return Taylor1(evaluated, n)
end

# ╔═╡ 21299896-05c5-4009-8110-4bff840a1884


# ╔═╡ cb0bc4ce-ae6f-49fb-8764-1c5fba58798b


# ╔═╡ 15b6c81f-f44f-4218-b3d3-945778418cd3
g(y) = sqrt(2/cbrt(4) - y)*(-pi3*cbrt(4)/12) + pi3/6

# ╔═╡ 81fdc85d-0e6c-40d7-b090-ecee3d0cb0d1
ginv(z) = 2/cbrt(4) - (12/(pi3*cbrt(4))*(z - pi3/6))^2

# ╔═╡ 10614db4-9315-433e-bd0e-1f5fbd0a6e5b


# ╔═╡ b94fb53f-745a-4495-a48d-ba96a7c5de67
collect(0:0.1:1) .|> g .|> ginv

# ╔═╡ 2712ce07-f715-42e7-8b75-11dd8fc836fe
begin
	zs = -4:0.01:4
	p = lines([NaN], [NaN])
	for z_0 in [0, 1//12]
		t = get_taylor(fp, z_0)
		lines!(zs, t.(zs .- z_0 * pi3))
		xlims!(-4, 4)
		ylims!(-4, 4)
	end
	p
...
# ╔═╡ bfc053c8-111f-4609-8e65-e92830d902ac


# ╔═╡ 05c5135c-42d4-4533-9730-6486d37bdcfe
sort(collect(keys(table)))

# ╔═╡ 193c5004-4461-4b7b-bf02-34781f45b899
n_plots = length(table)

# ╔═╡ e4852fac-7aaa-470d-8566-51d133db897a
begin
	fig = Figure()
	for (i_plot, z_0) in enumerate(table |> keys |> collect )
		t = get_taylor(fp, z_0)
		i, j = divrem(i_plot, 4)
		ax = Axis(fig[i, j]; title="$z_0 pi_3")
		plot!(ax, [1//6 * pi3], [2/cbrt(4)])
		lines!(ax, zs, t.(zs .- z_0 * pi3))
		xlims!(-4, 4)
		ylims!(-4, 4)
	end
	fig
...
# ╔═╡ ac861465-0844-4ad8-8b4b-f9a554803e26


# ╔═╡ b5efa594-736f-4ba0-b005-d45447c7876f


# ╔═╡ e601cfe8-6612-40e1-8ddc-8992e98779d0
begin
	zrs = -4:0.01:4 |> complex
	q = lines([NaN], [NaN])
	for z_0 in [-1//6, -1//12, 0, 1//12, 1//4, 1//3]
		t1 = get_taylor(fp, z_0)
		lines!(zrs |> real, t1.(zrs .- z_0 * pi3) |> real)
		xlims!(-4, 4)
		ylims!(-4, 4)
	end
	z_0 = 0
	t1 = get_taylor(fp, z_0)
	scatter!(zrs |> real, ginv.(g.(t1).(zrs .- z_0 * pi3)) |> real)
	plot!([1//6 * pi3], [2/cbrt(4)])
	q
end

# ╔═╡ 83ee4e95-ec87-42f7-ab63-c2409873f37d


# ╔═╡ 5736b969-c841-42fd-9204-51a3c6801cb7
lambda = collect(-pi:pi/100:pi)

# ╔═╡ d6d793df-02b7-450c-a244-d286d01b93a0
phi = collect(-pi/2:pi/100:pi/2)'

# ╔═╡ e01f86e1-a61d-4808-a2a2-29a4c857bea7
k = 2^(5//6) * exp.(im * lambda) .* tan.(pi/4 .- phi/2)

# ╔═╡ 837614ac-d1b0-40d3-b806-b57c62be6e3e
kflat = k |> Iterators.flatten |> collect

# ╔═╡ 914191fc-5e47-429f-b910-6f2486e82f7a
begin
	r = 20
	s = scatter(real(kflat), imag(kflat))
	xlims!(-r, r)
	ylims!(-r, r)
	s
end

# ╔═╡ c72b67b2-dca4-474c-a93e-ce931b813a63
7//12 * pi3

# ╔═╡ 67761432-d458-4b15-920e-5e20cd8122db
(
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "e7680b72976e9469d0e799cc3041782d9e980bed"
